...
import static org.psilynx.psikit.LogTable.LogValue;
import static org.psilynx.psikit.LogTable.LoggableType;

import org.psilynx.psikit.wpi.DataLogIterator;
import org.psilynx.psikit.wpi.DataLogReader;
import org.psilynx.psikit.wpi.DataLogRecord;
...
public class WPILOGReader implements LogReplaySource {
  ...
  public void start() {
    ...
    try {
      ...
      System.out.println("[PsiKit] Failed to open replay log file.");
    }

    // Check validity
    if (!reader.isValid()) {
      System.out.println("[PsiKit] The replay log is not a valid WPILOG" +
                      " file.");
      isValid = false;
    } else if (!reader.getExtraHeader().equals(WPILOGConstants.extraHeader)) {
      System.out.println(
          "[PsiKit] The replay log was not produced by PsiKit.");
      isValid = false;
    } else {
      isValid = true;
    }
          ...
          if (entry.equals(LogDataReceiver.timestampKey)) {
            ...
            switch (entryTypes.get(record.getEntry())) {
              ...
              case Boolean:
                ...
                break;
              case Integer:
                table.put(entry, new LogValue(record.getInteger(),
                        customType));
                break;
              case Float:
                table.put(entry, new LogValue(record.getFloat(),
                        customType));
                break;
              case Double:
